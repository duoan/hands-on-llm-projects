...
class TextGenerationCallback(TrainerCallback):
    def __init__(
        self, tokenizer, prompt="Once upon a time", max_length=100, log_to_wandb=True
    ):
...
class ParameterMonitorCallback(TrainerCallback):
    ...
    def on_optimizer_step(self, args, state, control, model=None, **kwargs):
        """
        Logs gradient norms at on optimizer step.
        """
        if model is None:
            return

        update_interval = self.log_interval * args.gradient_accumulation_steps
        if state.global_step % update_interval > 0:
            return
        ...
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is not None:
                ...
                if name.endswith("weight"):
                    ...
                    param_dists[f"param_dist/{name}"] = wandb.Histogram(
...
def main():
    ...
    os.makedirs(job_config.output_path, exist_ok=True)
    ...
    if local_rank == 0:
        wandb.login()
        wandb.init(
            project=f"hands-on-llm-pretrain-{job_config.model_path.replace('/', '-')}",
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        job_config.tokenizer_path, trust_remote_code=True
    )
    tokenizer.pad_token = tokenizer.eos_token
    tokenizer.pad_token_id = tokenizer.eos_token_id
    tokenizer.padding_side = "right"

    config = AutoConfig.from_pretrained(job_config.model_path, trust_remote_code=True)
    ...
    if not os.path.exists(job_config.tokenized_datapath):
        ...
        dataset.save_to_disk(job_config.tokenized_datapath)
    else:
        dataset = DatasetDict.load_from_disk(job_config.tokenized_datapath)
    ...
    trainer.save_model()
    tokenizer.save_pretrained(job_config.output_path)
    wandb.save(job_config.output_path)
